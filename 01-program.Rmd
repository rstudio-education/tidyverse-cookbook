...
This chapter includes the following recipes. To manipulate vectors with [purrr](http://purrr.tidyverse.org), see [Transform Lists].
...
You want to load a package that is in the tidyverse, so that you can use its contents. The package is [installed](#install) on your computer.
...
You want to ensure that you have installed the latest version of a package that is in the tidyverse. 
...
`update.packages()` will compare the version number of the package that you have installed locally to the newest version number available on CRAN. If your local version is older than the version on CRAN, `update.packages()` will download and install the newest version from CRAN. Otherwise, `update.packages()` will do nothing.
...
You want to ensure that you have installed the latest version of _every_ package that is in the tidyverse.
...
The chunk of functions connected by `%>%` is called a "pipe." Pipes are easier to read than nested code (mentally pronounce `%>%` as "then" to read the pipe as a sequence of steps). In addition, pipes require fewer temporary objects than code that passes temporary objects from one function to the next.

...
You want to apply a function separately to each element in a list and then combine the results into a single object.

This is similar to what you might do with a for loop, or with the apply family of functions.

For example, `got_chars` is a list of 30 lists. Each of the 30 sublists contains information about a different character in the _Game of Thrones_ television series. You would like to run the `pluck()` function separately on each sublist to extract the name element of the sublist, and then to combine the 30 names into a single list.
...
`map()` takes a vector to iterate over (here supplied by the pipe) followed by a function to apply to each element of the vector. Pass the function name to `map()` without quotes and without parentheses. `map()` will pass each element of the vector one at a time to the first argument of the function. 

If your function requires additional arguments to do its job, pass the arguments to `map()`. `map()` will forward these arguments in order, with their names to the function when it runs the function.
...
You can also use this syntax to map an arbitrary expression over a vector (instead of to map a predefined function),

```{r eval = FALSE}
gap_split %>% 
  map_lgl(~coef(lm(lifeExp ~ year, data = .x))[1] > 0)
```
